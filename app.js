/* =====================================================================
   PERTAMINA CONSTRUCTION DASHBOARD - APP.JS (Full, with PPT Cover Theme)
   ===================================================================== */
...
/* ------------------------------
   9.a) COVER THEME CONFIG
   ------------------------------ */
const BRAND = {
  coverBgPath: "assets/cover_weekly.png",
  logoLeftPath: "assets/logo_danan.png",
  logoRightPath: "assets/logo_pertamina.png",
  cWhite: "FFFFFF",
  cOverlay: "000000",
  cTitle: "FFFFFF",
  titleSize: 40,
...
function addCoverSlide(pptx, title, subTitle, periodText) {
  ...
  s.addImage({ path: BRAND.coverBgPath, x: 0, y: 0, w: "100%", h: "100%" });

  // Logo kiri (Danantara)
  s.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: 0.35, y: 0.25, w: 3.1, h: 0.9,
    fill: { color: BRAND.cWhite }, line: { color: "FFFFFF" }, rectRadius: 18
  });
  s.addImage({ path: BRAND.logoLeftPath, x: 0.55, y: 0.28, w: 2.7, h: 0.84, sizing: { type: "contain", w: 2.7, h: 0.84 } });

  // Logo kanan (Pertamina)
  s.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: 11.1, y: 0.25, w: 3.1, h: 0.9,
    fill: { color: BRAND.cWhite }, line: { color: "FFFFFF" }, rectRadius: 18
  });
  s.addImage({ path: BRAND.logoRightPath, x: 11.35, y: 0.28, w: 2.6, h: 0.84, sizing: { type: "contain", w: 2.6, h: 0.84 } });

  // Overlay transparan untuk teks
  s.addShape(pptx.shapes.RECTANGLE, {
    x: 2.4, y: 1.2, w: 10.9, h: 3.2,
    fill: { color: BRAND.cOverlay, transparency: 35 },
    line: { color: "FFFFFF", transparency: 100 }
  });

  s.addText(title || "Construction Weekly Report", {
    x: 2.8, y: 1.35, w: 10.2, h: 1.0,
    fontSize: BRAND.titleSize, bold: true, color: BRAND.cTitle
  });

  if (subTitle) {
    s.addText(subTitle, {
      x: 2.8, y: 2.15, w: 10.2, h: 0.9,
      fontSize: BRAND.subSize, bold: true, color: BRAND.cTitle
    });
  }

  if (periodText) {
    s.addText(`Period: ${periodText}`, {
      x: 2.8, y: 2.9, w: 10.2, h: 0.9,
      fontSize: BRAND.periodSize, bold: true, color: BRAND.cTitle
    });
  }
}

/* ------------------------------
   9.b) GENERATE POWERPOINT
   ------------------------------ */
function generatePowerPointReport(type, project, startDate, endDate) {
  try {
    ...
    s1.addText('Executive Summary', { x: 0.5, y: 0.5, w: 12, h: 1, fontSize: 28, bold: true, color: '1F4E5C' });
    ...
    s1.addText(`Period: ${periodText}`, { x: 0.5, y: 1.2, w: 12, h: 0.4, fontSize: 14, color: '626C71' });
    s1.addText(`Total Projects: ${totalProjects}`, { x: 0.5, y: 1.9, w: 12, h: 0.5, fontSize: 20 });
    s1.addText(`Average Progress: ${avgProgress}%`, { x: 0.5, y: 2.5, w: 12, h: 0.5, fontSize: 20 });
    s1.addText(`Average Safety Score: ${avgSafety}%`, { x: 0.5, y: 3.1, w: 12, h: 0.5, fontSize: 20 });
    s1.addText(`Active Permits: ${activePermits}`, { x: 0.5, y: 3.7, w: 12, h: 0.5, fontSize: 20 });
    s1.addText(`Generated: ${new Date().toLocaleString('id-ID')}`, { x: 0.5, y: 4.5, w: 12, h: 0.4, fontSize: 12, color: '626C71' });

    // PROJECT TABLE
    const s2 = pptx.addSlide();
    s2.addText('Project Details', { x: 0.5, y: 0.5, w: 12, h: 0.8, fontSize: 26, bold: true, color: '1F4E5C' });
    ...
    (filteredProjects.length ? filteredProjects : AppState.data.projects).slice(0, 8).forEach(p => {
      tableData.push([
        { text: p.Project_Name.replace(' Substation', ''), options: { fontSize: 12 } },
        { text: `${p.Progress_Percent}%`, options: { fontSize: 12 } },
        { text: `${p.Safety_Score}%`, options: { fontSize: 12 } },
        { text: `${p.Budget_Used_Percent}%`, options: { fontSize: 12 } }
      ]);
    });
    ...
    s3.addText('Safety Summary', { x: 0.5, y: 0.5, w: 12, h: 0.8, fontSize: 26, bold: true, color: '1F4E5C' });
    ...
    s3.addText(`Total Manpower: ${totalManpower.toLocaleString()}`, { x: 0.5, y: 1.6, w: 12, h: 0.6, fontSize: 20 });
    s3.addText(`Safe Man Hours: ${safeManHours.toLocaleString()}`, { x: 0.5, y: 2.3, w: 12, h: 0.6, fontSize: 20 });
    s3.addText(`Total Issues: ${totalIssues}`, { x: 0.5, y: 3.0, w: 12, h: 0.6, fontSize: 20 });
    s3.addText(`Total Plans: ${totalPlans}`, { x: 0.5, y: 3.7, w: 12, h: 0.6, fontSize: 20 });
...
function askQuestion(question) {
  ...
  setTimeout(() => {
    const botMessage = document.createElement('div'); botMessage.className = 'message bot-message'; botMessage.textContent = getBotResponse(question); messagesContainer.appendChild(botMessage); messagesContainer.scrollTop = messagesContainer.scrollHeight;
  }, 1000);
